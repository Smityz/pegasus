    ...
    if (request.sort_keys.empty()) {
        ...
        if (!request.reverse) {
            ...
            if (_data_cf_opts.prefix_extractor) {
                ...
                // for more details), and we have to do total order seek on rocksdb which might
                // be worse performance. However we consider that reverse scan is a rare use
                // case, and if your workload has many reverse scans, you'd better use 'common'
                // bloom filter (by set [pegasus.server]rocksdb_filter_type to 'common').
                                         ...
                                         ::dsn::rpc_replier<::dsn::apps::scan_response> &reply)
{
    ...
    if (_data_cf_opts.prefix_extractor) {
        ...
        if (start_hash_key.size() == 0) {
            ...
            // get_unordered_scanners) on a partition, we have to do total order seek on
            // rocksDB.
            rd_opts.total_order_seek = true;
            rd_opts.prefix_same_as_start = false;
        }
    ...
    if (request.hash_key_filter_type == ::dsn::apps::filter_type::FT_MATCH_PREFIX &&
        ...
        if (prefix_start.compare(start) > 0) {
            start = prefix_start;
            start_inclusive = true;
            // Now 'start' is generated by 'request.hash_key_filter_pattern', it may be not a
            // real hashkey, we should not seek this prefix by prefix bloom filter. However, it
            // only happen when do full scan (scanners got by get_unordered_scanners), in which
            // case the following flags has been updated.
    ...
    ::dsn::rpc_replier<::dsn::apps::hotkey_detect_response> &reply)
{
    ...
    if (args.operation == dsn::apps::hotkey_collector_operation::START) {
        ...
        derror("unkwnon hotkey_collector_operation");
