...
TEST(table_hotspot_policy, hotspot_algo_qps_variance)
{
    ...
    for (int i = 0; i < test_hotspot_calculator._points.size(); i++) {
        read_result[i] = test_hotspot_calculator._points[i].read_hotpartition_counter->get_value();
        write_result[i] =
            test_hotspot_calculator._points[i].write_hotpartition_counter->get_value();
    }
